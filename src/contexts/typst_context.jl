...
5. The global default [`context`](@ref)
...
| `io` | [`DefaultIO`](@ref) | A function with the signature `io()::IO`, which is used by `show_typst` when an `IO` is not otherwise specified. This is implemented as a function to avoid initialization errors. |
...
IOContext(io::IO, tc::TypstContext) = IOContext(io, :typst_context => tc)
