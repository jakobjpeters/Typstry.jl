...
1. The context given by keyword parameters or `TypstContext`, which are mutually exclusive
2. The context given by `IOContext(::IO, :typst_context => ::TypstContext)`
3. The type's default context, specified by implementing the `TypstContext` constructor
4. The global default [`context`](@ref)
...
| `context` | `TypstContext` | Used to override the context when formatting values in containers. |
...
| `io` | [`DefaultIO`](@ref) | A function with the signature `io()::IO`, which is used by `show_typst` when an `IO` is not otherwise specified. |
...
function show(io::IO, tc::TypstContext)
    ...
    if !isempty(tc)
        print(io, "; ")
        join_with(io, tc, ", ") do _io, (key, value)
            print(_io, key, " = ")
            show(_io, value)
        end
    end
